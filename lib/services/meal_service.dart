...
class MealService {
    ...
    try {
      ...
      final requestBody = {
        ...
        'items': mealLog.items.map((item) => {
          ...
          'estimated_portion_size': item.estimatedPortionSize,
      ...
      if (response.statusCode == 201) {
        final data = jsonDecode(response.body) as Map<String, dynamic>;
        final createdMeal = MealLog.fromJson(data['meal_log']);
  ...
  Future<Map<String, dynamic>?> getMealStats(int memberId, {int days = 7}) async {
    try {
      final uri = Uri.parse(
