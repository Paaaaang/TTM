...
class BadgeService {
  ...
  Future<List<Badge>> getAllBadges({bool forceRefresh = false}) async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${ApiConstants.baseUrl}/badges/'),
  ...
  Future<List<MemberBadge>> getMemberBadges(int memberId, {bool forceRefresh = false}) async {
    try {
      ...
      // API 호출
      final response = await http.get(
        Uri.parse('${ApiConstants.baseUrl}/badges/member/$memberId'),
        headers: {'Content-Type': 'application/json'},
      );
  ...
  Future<MemberBadge> awardBadge(int memberId, int badgeId) async {
    try {
      final response = await http.post(
        Uri.parse('${ApiConstants.baseUrl}/badges/award'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
          'member_id': memberId,
          'badge_id': badgeId,
  ...
  Future<BadgeStats> getBadgeStats(int memberId, {bool forceRefresh = false}) async {
    try {
      ...
      // API 호출
      final response = await http.get(
        Uri.parse('${ApiConstants.baseUrl}/badges/stats/$memberId'),
        headers: {'Content-Type': 'application/json'},
      );
