...
class _StatsScreenState extends State<StatsScreen> {
    ...
    try {
      ...
      if (isWeekly) {
        ...
        for (int i = 0; i < 7; i++) {
          ...
          // 해당 날짜의 기록 찾기
          final todayRecord = weightData
              .where((r) => r['date'] == dateStr)
              .toList();
          if (todayRecord.isNotEmpty) {
            weight = (todayRecord.first['weight'] as num).toDouble();
            ...
            for (var record in weightData) {
              ...
              if (recordDate.compareTo(dateStr) < 0) {
                foundWeight = (record['weight'] as num).toDouble();
              } else {
        ...
        for (int i = 0; i < 4; i++) {
          ...
          final weekData = await _loadWeekStats(
            _user!.memberId,
            weekStart,
            weekEnd,
          );
          ...
          if (weekWeights.isNotEmpty) {
            // 해당 주에 기록이 있으면 평균 사용
            weekWeight =
                weekWeights.reduce((a, b) => a + b) / weekWeights.length;
          } else {
    ...
    try {
      ...
      for (var type in mealTypes) {
        ...
        if (mealsByType.isNotEmpty) {
          // 가장 최근 식단만 선택
          final latestMeal = mealsByType.reduce(
  ...
  /// 주간 평균 통계 로드
  Future<Map<String, int>> _loadWeekStats(
    int userId,
    DateTime start,
    DateTime end,
  ) async {
  ...
  Future<Map<String, double>> _loadDayNutrition(
    int userId,
    DateTime date,
  ) async {
    final startOfDay = DateTime(date.year, date.month, date.day, 0, 0, 0);
  ...
  Future<Map<String, dynamic>> _buildNutritionReport(bool weekly) async {
    if (_user == null) {
      return {'score': 0.0, 'summaryLines': <String>[]};
    }
    ...
    for (final meal in meals) {
      ...
      final totals = dayTotals.putIfAbsent(
        dateStr,
    ...
    return {'score': avgScore, 'summaryLines': summaryLines};
  }
  ...
  Widget build(BuildContext context) {
    ...
    // 통계 계산
    final avgCalories = data.isEmpty
        ? 0
        : (data.fold<double>(
                    0,
                  ...
                  data.length)
              .round();
    final avgExercise = data.isEmpty
        ? 0
        : (data.fold<double>(
                    0,
                    (sum, item) => sum + (item['exercise'] as num).toDouble(),
                  ) /
                  data.length)
              .round();
    final weightChange = data.isEmpty
        ? 0.0
        : (data.last['weight'] as num).toDouble() -
              (data.first['weight'] as num).toDouble();

    // 칼로리 및 운동 변화량 계산 (마지막 - 첫번째)
    final caloriesChange = data.isEmpty
        ? 0.0
        : ((data.last['calories'] as num) - (data.first['calories'] as num))
              .toDouble();
    final exerciseChange = data.isEmpty
        ? 0.0
        : ((data.last['exercise'] as num) - (data.first['exercise'] as num))
              .toDouble();

    // 오늘 날짜 기준 최신 체중 (data의 마지막 값)
    final currentWeight = data.isEmpty
        ? (_user?.weightKg ?? 0.0)
        : (data.last['weight'] as num).toDouble();
              ...
              child: SingleChildScrollView(
                child: Column(
                  children: [
                    ...
                    // 칼로리 차트
                    _buildChartCard('칼로리 섭취', data, 'calories', Colors.orange),

                    const SizedBox(height: 16),

                    // 운동 차트
                    _buildChartCard('운동 칼로리', data, 'exercise', Colors.blue),

                    const SizedBox(height: 16),

                    // 체중 차트
                    _buildChartCard('체중 변화', data, 'weight', Colors.green),
                      ...
                      lineBarsData: [
                        LineChartBarData(
                          spots: data.asMap().entries.map((entry) {
                            return FlSpot(
                              entry.key.toDouble(),
                              (entry.value[dataKey] as num).toDouble(),
                            );
                          }).toList(),
                          isCurved: true,
                          color: color,
  ...
  double _getMinY(List<Map<String, dynamic>> data, String dataKey) {
    ...
    final values = data.map((e) => (e[dataKey] as num).toDouble()).toList();
