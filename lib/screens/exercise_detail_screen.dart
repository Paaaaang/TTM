...
class ExerciseDetailScreen extends StatelessWidget {
  ...
  const ExerciseDetailScreen({Key? key, required this.workout})
    : super(key: key);
          ...
          ...effects.asMap().entries.map((entry) {
            return Padding(
              padding: EdgeInsets.only(
                bottom: entry.key < effects.length - 1 ? 12 : 0,
              ),
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
  ...
  List<String> _getWorkoutEffects(String workoutType) {
    final effectsMap = {
      '걷기': ['심혈관 건강 개선', '스트레스 감소', '체중 관리', '관절 부담 최소화'],
      '런닝': ['심폐 지구력 향상', '체지방 감소', '뼈 강화', '정신 건강 개선'],
      '자전거': ['하체 근력 강화', '유산소 능력 향상', '관절 보호', '칼로리 소모 효과'],
      '수영': ['전신 근육 사용', '체중 부담 없음', '심폐 기능 향상', '유연성 증가'],
      '웨이트 트레이닝': ['근육량 증가', '기초대사량 향상', '골밀도 증가', '자세 개선'],
      '푸시업': ['상체 근력 강화', '코어 안정성 향상', '자세 교정', '언제 어디서나 가능'],
      '요가': ['유연성 증가', '스트레스 완화', '자세 개선', '집중력 향상'],
      '스트레칭': ['근육 이완', '혈액 순환 개선', '부상 예방', '관절 가동범위 증가'],
    };

    return effectsMap[workoutType] ?? ['신체 활동 증가', '칼로리 소모', '건강 증진'];
  }
}
